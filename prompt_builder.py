...
[상품 병합 케이스 — 첨부 이미지가 '기존 상품소개' 자료일 때]
첨부 이미지가 풍경 사진이 아니라 기존에 만들어둔 다른 상품소개 자료(옛 기획안 슬라이드
캡처 등, 텍스트 위주)라면, 이번 요청은 기존 상품 2개를 하나의 새로운 상품으로 재구성하는
"병합" 케이스일 수 있습니다. 위 텍스트 자료(워드 파일)가 그 병합 방식(둘 중 무엇을
가져오고, 어떤 순서/테마로 새로 짤지)을 지시하는 "설계도" 역할을 하고, 각 이미지는 그
설계도가 참조하는 기존 상품 하나하나의 원본 설명(목적지, 일정, 하이라이트 등)입니다.
이런 경우 아래 원칙을 따르세요:
- 워드 파일의 지시를 최우선으로 따라 새 상품의 정체성(상품명, 컨셉, 최종 일정 구성)을
  정하세요. 이미지 속 옛 상품의 정체성(옛 상품명, 옛 카피)을 그대로 새 상품에 가져오지
  마세요 — 새 상품은 완전히 새로운 하나의 상품입니다.
- 두 이미지를 "상품 A 소개 + 상품 B 소개"처럼 순서대로 이어 붙이거나 나열하지 마세요.
  워드 파일이 지시한 순서/테마로 목적지를 재배열해 하나의 여정처럼 자연스럽게
  구성하세요. destinations[].region_tag(방문지 그룹 라벨)를 활용해 "구간 1(옛 상품A 코스)"
  "구간 2(옛 상품B 코스)"처럼 원래 어느 상품 출신인지 보여주거나, 워드 파일이 지역/테마
  기준의 새로운 묶음을 지시했다면 그 기준으로 묶으세요 — 어느 쪽이든 destinations 배열
  안에서 같은 그룹끼리는 반드시 붙어 있어야 합니다.
- guide_profile, experience_points, safety_note, meal_info, season처럼 상품 전반에 걸쳐
  하나만 있어야 하는 공통 섹션은 두 이미지에서 겹치는 내용을 그대로 두 번 이어붙이지
  말고, 새 상품 하나에 맞게 합쳐서 한 번만 작성하세요(예: 두 상품의 인솔자가 다르면
  워드 파일에 정리된 새 상품 담당자만 반영). 두 원본에만 각각 있던 서로 다른 정보(예:
  한쪽에만 있는 고도 프로필)는 새 일정에 실제로 해당할 때만 가져오세요.
- 두 이미지 어디에도 없고 워드 파일에도 없는 연결 서사(예: "왜 이 두 지역을 함께
  가는지"에 대한 임의의 이유)를 지어내지 마세요 — 그 연결 이유가 워드 파일에 명시돼
  있으면 그대로 쓰고, 없으면 배경 설명은 사실 관계만 담담히 서술하세요.""" if has_images else ""}
"""
    return prompt


def build_mobile_system_prompt(category):
    """'모바일 최적화 기획안' 전용 시스템 프롬프트.

    일반 build_system_prompt와 근본적으로 다른 점: 사업부 원본자료(워드)가 아니라
    "이미 만들어진 PC용 상품소개 이미지"가 유일한 입력이고, 목표는 새로운 카피를
    창작하는 게 아니라 그 안의 내용을 손실 없이 그대로 옮기는 것이다.

    [2026-09-10 수정 — "축약 절대 금지" 원칙을 "항목 삭제/병합 금지, 문장 축약은
    허용"으로 정정]
    처음엔 "모바일 최적화 콘텐츠 가이드라인_20260702.pdf"(마케팅실 최정인)와 돌로미테
    Before/After 샘플을 대조해 "문장 한 글자도 축약 금지, 재배치만 허용"으로 지시를
    짰었다. 그런데 실제로 "미서부 대장정(요세미티+옐로스톤+그랜드서클)" 상품으로
    돌려보니, AI가 그랜드캐년의 "림 트레일"/"브라이스엔젤 트레일"/"투 트레일"처럼
    원본에 각각 개별 카드로 있던 하위 코스 3개를 "그랜드 캐년 국립공원" 카드 하나로
    뭉뚱그려 요약해버리는 문제가 나왔다 — 문장은 축약 수준이 아니라 아예 카드/항목
    자체가 사라진 것과 같다. 담당자(정현지) 확인 결과, 실제 운영 방침은: "내용을
    없애는 게 아니라 축약하는 것" — 즉 사업부가 한 상품소개 1벌을 PC/모바일 양쪽에
    다 쓰므로, 원본에 있던 모든 항목(트레일/코스/스팟/카드 하나하나)은 빠짐없이
    다 있어야 하고, 다만 각 항목 "안의" 문장 표현은 모바일 가독성을 위해 간결하게
    다듬어도 된다(수치·고유명사 같은 사실 정보는 유지). 이번 수정은 그 구분을
    명확히 하는 데 집중한다: (1) 원본의 개별 하위 항목은 절대 병합·삭제 금지 —
    스키마 배열(destinations 등) 원소 개수가 원본 카드 개수와 1:1로 맞아야 함,
    (2) 각 원소 안의 설명 문장은 축약 가능.

    스키마는 일반 모드와 동일한 SCHEMA_HINTS["정현지"]를 그대로 재사용한다 —
    build_mobile()이 이 스키마를 그대로 받아 모바일 스타일로 그리도록 만들어져
    있다."""
    schema = SCHEMA_HINTS["정현지"]

    prompt = f"""역할: 당신은 혜초여행사 콘텐츠팀에서 "모바일 최적화 기획안"을 만드는 담당자입니다.

[이번 작업의 특수성 — 반드시 읽으세요]
이번 입력은 사업부가 새로 작성한 원고가 아니라, 이미 완성돼 실제로 쓰이고 있는
PC용(웹) 상품소개 이미지입니다. 이미지는 보통 세로로 긴 카드뉴스 형태이며, 1장
또는 여러 장으로 나뉘어 첨부되어 있을 수 있습니다(같은 상품의 이어지는 페이지들).
당신의 역할은 새로운 카피를 창작하는 게 아니라, 이 이미지들 안에 있는 정보를
빠짐없이 옮기되 모바일 화면에 맞게 문장만 간결하게 다듬는 것입니다.

[절대 원칙 1 — 항목(카드) 삭제·병합 절대 금지]
회사의 실제 운영 방침: 상품소개는 PC/모바일 양쪽에 같은 1벌을 씁니다. 그래서
"내용을 없애는 게 아니라 축약하는 것"이 핵심입니다 — 문장 표현을 간결하게
다듬는 건 좋지만, 원본에 있던 항목 자체를 줄이거나 여러 개를 하나로 합쳐서
요약하면 절대 안 됩니다.
- 원본 이미지에서 소제목/카드/트레일/코스/스팟이 각각 따로 등장하면
  (예: "그랜드 캐년 국립공원" 아래 "림 트레일", "브라이스엔젤 트레일", "투
  트레일"이 각각 자기 사진·자기 설명·자기 거리·소요시간을 가진 별개 카드로
  있다면), destinations(또는 tour_spots) 배열에도 반드시 그 개수만큼
  개별 원소로 각각 만드세요. "그랜드 캐년 국립공원" 하나로 뭉쳐서 트레일 이름만
  나열하고 설명은 합쳐버리는 건 금지된 요약입니다 — 실제로 이런 병합 실수가
  한 번 발생해서 이 규칙을 강하게 명시합니다.
- 이때 region_tag에 상위 묶음(예: "그랜드 캐년 국립공원")을 넣어서, 하위 항목들이
  같은 그룹으로 묶여 보이게 하세요 — "상위 카테고리(국립공원/지역) → 개별
  항목(트레일/코스)"의 2단 구조를 destinations[].region_tag로 표현하는 것이지,
  하위 항목을 지우고 상위 카테고리 하나로 축소하라는 뜻이 아닙니다.
- feature_points/experience_points, why_reasons, meal_info/safety_note의
  Q&A처럼 다른 배열형 필드도 마찬가지입니다 — 원본에 개별 항목이 N개 있으면
  스키마에도 N개를 각각 만드세요.
- 원본에 명확히 등장하는 수치(거리/소요시간/고도/연박 수 등), 고유명사(트레일명·
  코스명·지명), 총 거리/소요시간 같은 구체적 사실은 어느 항목에서 왔든 절대
  누락하면 안 됩니다 — 각 항목 자체의 description 안에 그대로 남겨두세요(다른
  섹션에 따로 옮겨놓는 것으로 대체하지 마세요).

[절대 원칙 2 — 문장 축약은 허용, 단 사실 손실 없이]
위 원칙 1과 달리, 각 항목 "안의" 설명 문장 자체는 모바일 가독성을 위해 간결하게
다듬어도 됩니다. 원본이 장황하게 서술한 문장을 그대로 베낄 필요는 없고, 핵심
정보(어디를, 무엇을, 얼마나)가 다 들어간 짧고 읽기 쉬운 문장으로 정리하세요.
다만:
- 압축하는 과정에서 수치·고유명사·핵심 포인트가 빠지면 안 됩니다.
- "핵심만 남기고 나머지 항목 자체를 버린다"는 원칙 1 위반입니다 — 이 원칙 2는
  "각 항목의 문장 길이"에 대한 것이지 "항목 개수"에 대한 것이 아닙니다.
- 오탈자 교정이나 "이미지 A/B/C..." 같은 내부 표시 제거처럼, 의미를 바꾸지 않는
  정돈은 항상 허용됩니다.

[여러 장 이미지 처리]
이미지가 여러 장이면 같은 상품의 이어지는 페이지로 보고 순서대로 하나의 스키마
객체로 합쳐서 채우세요. 페이지 경계 때문에 문장이나 카드가 잘려 있으면 자연스럽게
이어 붙이되, 이때도 원칙 1(항목 병합 금지)은 그대로 적용됩니다 — 페이지가
나뉘어 있다고 그 페이지 안의 여러 카드를 하나로 합치면 안 됩니다.

세로로 긴 원본 이미지 1장이 시스템에 의해 여러 조각으로 나뉘어 전달되는 경우,
그 앞에 "[첨부 이미지 N: ... 조각으로 나눠 전달합니다]" 같은 안내 텍스트가
붙어 있습니다. 이 안내가 있는 조각들은 원래 하나로 이어지던 같은 페이지가
기술적으로 잘려서 온 것뿐이니, 페이지가 바뀐 게 아니라 "글자가 이어지는
중간"이라고 생각하고 이어서 읽으세요 — 특히 이미지 하단부(예: 마지막 조각)에
있는 항목을 앞부분보다 소홀히 다루거나 빠뜨리지 않도록 주의하세요. 실제로
"몬테로사" 같은 마지막 구간이 통째로 누락된 사례가 있었습니다.

이미지 상단에 큰 대제목 위/옆에 짧은 보조 태그나 배지 문구(예: "당일 산행으로
부담없이!", "폭발적 문의!")가 별도로 보인다면, 버리지 말고 해당 heading류
필드(destinations_heading, tour_spots_heading 등) 맨 앞줄에 줄바꿈으로 붙여서
살리세요 (예: "당일 산행으로 부담없이!\\n돌로미테에서 놓쳐선 안되는 트레킹 코스
Best 6"). 이 시스템은 그 줄바꿈을 배지 스타일로 렌더링합니다.

사진 자체는 옮길 필요 없습니다(이 시스템은 사진 자리에 자리표시 박스만 넣고,
실제 사진은 디자이너가 나중에 앉힙니다) — 다만 사진에 대한 설명/캡션 텍스트가
있다면 그건 description 등에 반영하세요.

이미지에서 읽을 수 없을 만큼 흐릿하거나 잘린 부분이 있으면, 그 부분만 비워두고
나머지는 정상적으로 채우세요. 없는 내용을 지어내지 마세요.
...
[출력 형식]
반드시 JSON으로만 응답하세요. 다른 텍스트를 포함하지 마세요. 응답은 반드시 중괄호
{{ }}로 시작하는 JSON 객체 하나여야 합니다 — 대괄호로 감싼 배열이 아닙니다.

[중요 — 모든 필드 공통 규칙]
아래 스키마의 모든 값은 실제 고객이 보게 될 화면에 그대로 노출됩니다. 어떤 필드에도
이 작업/문서 자체에 대한 메타 설명("모바일 최적화 기획안입니다", "AI가 추출한" 등)을
쓰지 마세요.

스키마 (이 구조를 정확히 따르세요. 필드를 빼거나 이름을 바꾸지 마세요. 이미지에
해당 정보가 아예 없는 필드는 빈 문자열/빈 배열로 두세요 — 없는 내용을 새로
지어내면 안 됩니다):
{schema}

{DESTINATIONS_RULE}
위 규칙에 이어서 — "실제로 등장하는 개수만큼"이란 각 국립공원/지역별로 뭉뚱그린
개수가 아니라, 원본에 개별 카드로 등장하는 트레일/코스/스팟 하나하나를 센 개수를
말합니다. 예: 국립공원 3곳 각각에 트레일이 2~3개씩 있으면 destinations는
6~9개가 되어야지, 국립공원당 1개씩 3개가 되면 안 됩니다.

[첨부 이미지]
이 메시지에는 모바일 최적화 대상인 기존 PC용 상품소개 이미지가 첨부되어 있습니다.
위 [절대 원칙 1], [절대 원칙 2]에 따라 이 이미지들의 항목을 하나도 빠짐없이
스키마에 옮기고, 문장은 모바일에 맞게 간결히 정리하세요.
"""
    return prompt
