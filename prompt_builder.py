...
def build_mobile_system_prompt(category):
    ...
    이 지시는 "모바일 최적화 콘텐츠 가이드라인_20260702.pdf"(마케팅실 최정인,
    2026.07.02)와 실제 완성 Before/After 샘플(17.모바일최적화 폴더 — 돌로미테,
    그랜드서클 등)을 대조 확인해 작성했다. 실제 샘플에서 확인된 핵심 사실: 두
    버전의 문단·수치·캡션 텍스트는 100% 동일했고, 달라진 건 폰트 크기·여백·배지
    (태그) 스타일·배경색뿐이었다 — 즉 "요약/축약"이 아니라 "재배치". 그 시각
    스타일 차이는 builder.py의 build_mobile()이 코드로 처리하므로, 이 프롬프트는
    AI에게 요약하지 말라고 강하게 못 박는 데 집중한다. 스키마는 일반 모드와 동일한
    SCHEMA_HINTS["정현지"]를 그대로 재사용한다 — build_mobile()이 이 스키마를
    그대로 받아 모바일 스타일로 그리도록 만들어져 있다."""
...
당신의 역할은 새로운 카피를 창작하는 게 아니라, 이 이미지들 안에 있는 모든 텍스트
정보를 정확히 읽어내 아래 스키마 형태로 옮기는 것입니다.

[절대 원칙 — 요약·축약 금지, "재배치"만 허용]
회사의 모바일 최적화 가이드라인은 이렇게 명시합니다: "콘텐츠의 양과 깊이는
타협하지 않으며, 정보 구조를 모바일 UX에 맞게 재배치하는 방식으로 진행한다."
즉 이 작업은 내용을 줄이는 작업이 아닙니다. 실제 완성 샘플을 대조해봐도 PC판과
모바일판의 문장·수치·설명은 완전히 동일했고 다른 건 디자인(폰트 크기·여백·배지
스타일·배경색)뿐이었습니다 — 그 디자인 차이는 이 시스템이 자동으로 처리하므로
신경 쓰지 마세요. 당신은 오직 아래만 하면 됩니다:
- 이미지에 보이는 모든 텍스트(제목, 소제목, 본문 설명, 총 거리/소요시간 같은
  수치, 코스 경로, 캡션, Q&A, 날씨 안내 등)를 빠짐없이 읽어 스키마 필드에 옮기세요.
  "핵심만 뽑아서" 짧게 줄이지 마세요 — 문장을 압축·생략하면 안 됩니다.
- 오탈자 교정이나 "이미지 A/B/C..." 같은 내부 표시 제거처럼, 의미를 바꾸지 않는
  최소한의 정돈만 허용됩니다.
- 이미지가 여러 장이면 같은 상품의 이어지는 페이지로 보고 순서대로 하나의
  스키마 객체로 합쳐서 채우세요. 페이지 경계 때문에 문장이 잘려 있으면 자연스럽게
  이어 붙이세요.
...
- 사진 자체는 옮길 필요 없습니다(이 시스템은 사진 자리에 자리표시 박스만
  넣고, 실제 사진은 디자이너가 나중에 앉힙니다) — 다만 사진에 대한 설명/캡션
  텍스트가 있다면 그건 description 등에 반영하세요.
...
원래 문장을 최대한 그대로 옮기세요. 다만 문장이 끊겨 있거나 어색하게 줄바꿈된
부분은 자연스러운 완성 문장으로만 정리하세요 (내용 추가·삭제 금지).
...
위 [절대 원칙]에 따라 이 이미지들의 내용을 빠짐없이, 축약 없이 읽어 스키마에
옮기세요.
"""
    return prompt
