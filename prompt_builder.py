...
※ destinations[].region_tag — 방문지 그룹 라벨: 방문지가 여러 개일 때, 성격이 비슷한
  것끼리(예: 같은 문명/테마) 또는 같은 지역/도시에 있는 것끼리 묶어서 표시하기 위한
  소제목입니다. 방문지가 나열식으로만 쭉 이어지면 읽기 피곤하다는 피드백이 있었습니다 —
  사업부 자료에 이미 그룹 구조가 있다면(예: "[ 사포테카 문명 | 와하카 ]", "[ 식민 기획도시 |
  과달라하라·과나후아토·케레타로 ]"처럼 테마와 지역을 함께 묶어놓은 괄호 표기) 그 구조를
  그대로 살려서 region_tag에 짧은 그룹명으로 담으세요(예: "사포테카 문명 · 와하카"). 테마
  구분이 없고 지역/도시만 구분된다면 지역명만 써도 됩니다(예: "와하카"). 반드시 지켜야 할
  규칙: 같은 그룹에 속하는 방문지는 destinations 배열에서 반드시 서로 붙어 있어야
  합니다(그룹 A의 방문지를 전부 나열한 다음에 그룹 B로 넘어가는 식) — 그룹이 배열 중간에
  흩어져 있으면 화면에 같은 그룹명이 여러 번 끊겨서 나옵니다. 방문지들이 서로 뚜렷하게
  구분되지 않거나(예: 전부 한 도시 안의 개별 명소) 사업부 자료에 그룹 지을 근거가 없으면,
  억지로 묶지 말고 전체 region_tag를 빈 문자열로 두세요.
